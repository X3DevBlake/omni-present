...
export default function PhysicsSimulation3D() {
  const [objects, setObjects] = useState([
    { id: 1, type: 'sphere', position: [-3, 5, 0], velocity: [2, 0, 1], mass: 1, color: '#00f5ff' },
    { id: 2, type: 'box', position: [3, 7, 0], velocity: [-1, 0, -1], mass: 1.2, color: '#a855f7' },
    { id: 3, type: 'torus', position: [0, 9, 3], velocity: [0.5, 0, -0.5], mass: 0.8, color: '#ec4899' },
    { id: 4, type: 'sphere', position: [-2, 6, -3], velocity: [1, 0, 2], mass: 0.9, color: '#10b981' }
  ]);

  const [gravity, setGravity] = useState(9.8);
  const [forceField, setForceField] = useState(false);
      ...
      <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-4">
          ...
          <div className="flex items-end gap-2">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={addObject}
              className="px-4 py-2 rounded-lg font-semibold bg-cyan-500/20 border border-cyan-500/50 text-cyan-400 flex-1"
            >
              + Add Object
            </motion.button>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4 mt-4">
          <div className="bg-white/5 border border-white/10 rounded-lg p-3">
            <p className="text-white/60 text-xs mb-1">Active Objects</p>
            <p className="text-white font-bold text-xl">{objects.length}</p>
          </div>
          <div className="bg-white/5 border border-white/10 rounded-lg p-3">
            <p className="text-white/60 text-xs mb-1">Gravity</p>
            <p className="text-white font-bold text-xl">{gravity.toFixed(1)} m/s²</p>
          </div>
          <div className="bg-white/5 border border-white/10 rounded-lg p-3">
            <p className="text-white/60 text-xs mb-1">Status</p>
            <p className="text-white font-bold text-xl">{simulating ? '🟢 Active' : '⏸️ Paused'}</p>
          </div>
        </div>
      </div>
    </div>
